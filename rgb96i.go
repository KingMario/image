...
func NewRGB96i(r image.Rectangle) *RGB96i {
	return new(RGB96i).Init(make([]uint8, 3*r.Dx()*r.Dy()), 3*r.Dx(), r)
}
...
func (p *RGB96i) BaseType() image.Image { return p }
...
func (p *RGB96i) Depth() reflect.Kind   { return reflect.Uint8 }

func (p *RGB96i) ColorModel() color.Model { return color.RGBAModel }
...
func (p *RGB96i) RGBAt(x, y int) [3]uint8 {
...
func (p *RGB96i) SetRGB(x, y int, c [3]uint8) {
	if !(image.Point{x, y}.In(p.M.Rect)) {
		return
	}
	i := p.PixOffset(x, y)
...
func (p *RGB96i) CopyFrom(m image.Image) *RGB96i {
	panic("TODO")
}
