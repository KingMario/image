...
func NewRGB48(r image.Rectangle) *RGB48 {
	return new(RGB48).Init(make([]uint8, 1*r.Dx()*r.Dy()), 1*r.Dx(), r)
}
...
func (p *RGB48) Depth() reflect.Kind   { return reflect.Uint8 }
...
func (p *RGB48) RGB48At(x, y int) colorExt.RGB48 {
	...
	i := p.PixOffset(x, y)
	return *(*colorExt.RGB48)(unsafe.Pointer(&p.M.Pix[i]))
}
...
func (p *RGB48) Set(x, y int, c color.Color) {
	...
	i := p.PixOffset(x, y)
	c1 := p.ColorModel().Convert(c).(colorExt.RGB48)
	*(*colorExt.RGB48)(unsafe.Pointer(&p.M.Pix[i])) = c1
	return
}
...
func (p *RGB48) SetRGB48(x, y int, c colorExt.RGB48) {
	...
	i := p.PixOffset(x, y)
	*(*colorExt.RGB48)(unsafe.Pointer(&p.M.Pix[i])) = c
	return
}
