...
func NewGrayA128i(r image.Rectangle) *GrayA128i {
	return new(GrayA128i).Init(make([]uint8, 2*r.Dx()*r.Dy()), 2*r.Dx(), r)
}
...
func (p *GrayA128i) BaseType() image.Image { return p }
...
func (p *GrayA128i) ColorModel() color.Model { return color.RGBAModel }
...
func (p *GrayA128i) GrayAAt(x, y int) [2]uint8 {
...
func (p *GrayA128i) SetGrayA(x, y int, c [2]uint8) {
	if !(image.Point{x, y}.In(p.M.Rect)) {
		return
	}
	i := p.PixOffset(x, y)
...
func (p *GrayA128i) CopyFrom(m image.Image) *GrayA128i {
	panic("TODO")
}
