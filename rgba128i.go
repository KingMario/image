...
func NewRGBA128i(r image.Rectangle) *RGBA128i {
	return new(RGBA128i).Init(make([]uint8, 4*r.Dx()*r.Dy()), 4*r.Dx(), r)
}
...
func (p *RGBA128i) BaseType() image.Image { return p }
...
func (p *RGBA128i) Depth() reflect.Kind   { return reflect.Float32 }

func (p *RGBA128i) ColorModel() color.Model { return color.RGBA64Model }
...
func (p *RGBA128i) RGBA128iAt(x, y int) [4]float32 {
...
func (p *RGBA128i) SetRGBA128i(x, y int, c [4]float32) {
	if !(image.Point{x, y}.In(p.M.Rect)) {
		return
	}
	i := p.PixOffset(x, y)
...
func (p *RGBA128i) CopyFrom(m image.Image) *RGBA128i {
	panic("TODO")
}
